...
export const RiskDashboard: React.FC<RiskDashboardProps> = ({
  ...
  const riskClasses = getRiskColor(
  ...
  const riskDistribution = [
    {
      name: 'Critical',
      value: risk_summary.critical_count || 0,
      color: '#EF4444',
    },
    {
      name: 'High',
      value: risk_summary.high_count || 0,
      color: '#F97316',
    },
    {
      name: 'Medium',
      value: risk_summary.medium_count || 0,
      color: '#F59E0B',
    },
    {
      name: 'Low',
      value: risk_summary.low_count || 0,
      color: '#3B82F6',
    },
    {
      name: 'Info',
      value: risk_summary.informational_count || 0,
      color: '#94A3B8',
    },
  ].filter((item) => item.value > 0);

  const totalRiskItems = riskDistribution.reduce(
  ...
  return (
    <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-800 rounded-2xl p-6 mt-6 shadow-2xl relative overflow-hidden">
      ...
      <div
        className={`absolute top-0 right-0 w-96 h-96 bg-opacity-20 rounded-full blur-3xl -z-10 transition-colors ${
          risk_summary.overall_risk === 'Critical'
            ? 'bg-rose-600'
            : risk_summary.overall_risk === 'High'
            ? 'bg-orange-600'
            : risk_summary.overall_risk === 'Moderate' ||
      ...
      <div className="flex items-center justify-between mb-8 border-b border-slate-800 pb-4">
        ...
        <div>
          ...
          <p className="text-slate-400 mt-1 text-sm">
            Risk summary derived from the uploaded security assessment report
          </p>
        </div>
      ...
      {risk_summary.exposed_ports === 0 ? (
        ...
        <div className="text-center py-12 text-slate-400 bg-slate-800/30 rounded-xl border border-slate-700/50">
          ...
          <h3 className="text-lg font-medium text-slate-300">
            No exposed ports were identified in this report.
          </h3>

          <p className="text-sm mt-2">
            The risk assessment requires exposed services to calculate an attack surface.
          </p>
        ...
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div
              className={`col-span-1 lg:col-span-1 border rounded-2xl p-6 flex flex-col justify-center items-center text-center ${riskClasses} relative overflow-hidden shadow-lg`}
            >
              ...
              <div className="w-full bg-slate-900/50 rounded-full h-3 mb-6 overflow-hidden border border-slate-700/30">
                ...
                <div
                  className={`h-3 rounded-full ${
                    risk_summary.risk_score >= 70
                      ? 'bg-rose-500'
                      : risk_summary.risk_score >= 40
                      ? 'bg-orange-500'
                      : risk_summary.risk_score >= 20
                      ? 'bg-amber-500'
                      : 'bg-emerald-500'
                  }`}
                  style={{
                    width: `${Math.min(
                      Math.max(risk_summary.risk_score, 0),
                      100
                    )}%`,
                  }}
                />
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                ...
                <div className="bg-slate-900/50 border border-slate-700/50 rounded-xl p-4 flex flex-col items-center justify-center text-center hover:bg-slate-800/50 transition">
                  ...
                  <span className="text-2xl font-bold text-slate-100">
                    {attack_surface.total_open_ports}
                  </span>
                ...
                <div className="bg-slate-900/50 border border-slate-700/50 rounded-xl p-4 flex flex-col items-center justify-center text-center hover:bg-slate-800/50 transition">
                  ...
                  <span className="text-2xl font-bold text-slate-100">
                    {attack_surface.unique_services}
                  </span>
                ...
                <div className="bg-slate-900/50 border border-slate-700/50 rounded-xl p-4 flex flex-col items-center justify-center text-center hover:bg-slate-800/50 transition">
                  ...
                  <span className="text-2xl font-bold text-slate-100">
                    {attack_surface.remote_access_services}
                  </span>
                ...
                <div className="bg-slate-900/50 border border-slate-700/50 rounded-xl p-4 flex flex-col items-center justify-center text-center hover:bg-slate-800/50 transition">
                  ...
                  <span className="text-2xl font-bold text-slate-100">
                    {attack_surface.database_services}
                  </span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                {riskBreakdown.map((risk) => (
                  ...
                  <div
                    key={risk.name}
                    className={`bg-slate-900/40 border-l-4 ${risk.border} rounded-r-lg p-4 flex items-center justify-between`}
                  >

                    <span
                      className={`text-xs font-bold tracking-wider ${risk.text}`}
                    >
              ...
              {priority_findings.length === 0 ? (
                ...
                <div className="space-y-3">
                  ...
                  {priority_findings.map((finding) => (
                    ...
                    <div
                      key={finding.rank}
                      className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-4 hover:bg-slate-800/60 transition group cursor-default"
                    >
                      ...
                      <div className="flex justify-between items-start mb-2">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <span
                            className={`text-xs font-bold uppercase tracking-wider px-2 py-0.5 rounded border bg-slate-900/50 ${
              ...
              {recommendations.length === 0 ? (
                ...
                <div className="space-y-3">
                  ...
                  {recommendations.map((rec) => (
                    ...
                    <div
                      key={rec.priority}
                      className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-4 flex gap-4"
                    >

                      <div className="flex-shrink-0 mt-0.5">

                        <div
                          className={`text-xl font-black opacity-20 ${getSeverityColorText(
                            rec.severity
                          )}`}
                        >
                          {rec.priority
                      ...
                      <div className="flex-grow">
                        ...
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span
                            className={`text-[10px] font-bold uppercase tracking-wider ${getSeverityColorText(
                              rec.severity
                            )}`}
                          >
                            — {rec.severity}
                        ...
                        <div className="flex flex-wrap gap-2">
                          ...
                          {rec.related_ports.map(
                            (p, i) => (

                              <span
                                key={i}
                                className="text-[10px] bg-slate-900 border border-slate-700 text-slate-300 px-1.5 py-0.5 rounded font-mono"
                              >
                                {p}
                              </span>
